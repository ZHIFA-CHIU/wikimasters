...
async function main() {
  try {
    ...
    if (users.length === 0) {
      console.log("⚠️ No users found in usersSync. Skipping article seed.");
      return;
    }
    const ids = users.map((user) => user.id);
    const seedCount = SEED_COUNT;
    console.log(`👥 Using ${users.length} user(s)`);
    console.log(`🧮 Generating ${seedCount} article(s)`);

    // Create a round-robin distribution of user IDs for articles
    const authorIds = Array.from(
      { length: seedCount },
      (_, i) => ids[i % ids.length],
    );
    console.log(
      `📊 Articles will be evenly distributed among ${users.length} user(s)`,
    );
    ...
    console.log(`✅ Inserted ${seedCount} article(s) into the database\n`);
    ...
    try {
      await db.execute(
        sql`SELECT setval(pg_get_serial_sequence('articles','id'), COALESCE((SELECT MAX(id) FROM articles), 1), true);`,
      );
